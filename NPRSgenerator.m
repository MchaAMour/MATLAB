function y = NPRSgenerator(nS, CellID)
...
persistent hInt2Bit;
% Assumed parameters
NcellID = CellID; % One of possible 504 values
Ncp = 0; % for normal CP, or 0 for Extended CP
NmaxDL_RB = 1; % largest downlink bandwidth configuration, in resource blocks
y = complex(zeros(NmaxDL_RB*2, 2, 2, 1));
...
c_init = ((2^28)*(NcellID/512))+(2^10)*(7*((nS+i-1)+1)+l(lIdx)+1)*((2* (mod(NcellID,512)))+1)+ (2* (mod(NcellID,512))) + Ncp;
